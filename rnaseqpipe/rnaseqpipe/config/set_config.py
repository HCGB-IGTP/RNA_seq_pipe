...
from rnaseqpipe.config import extern_progs
...
def check_dependencies(Debug):
	"""
	Check if available the different software required for ``rnaseqpipe`` execution.
	
	Using the function :func:`rnaseqpipe.config.extern_progs.read_dependencies` the 
	information for all the dependencies is retrieved from file :file:`rnaseqpipe/config/software/dependencies.csv`.
	
	For each software, the function :func:`rnaseqpipe.config.set_config.get_exe` retrieves
	whether it is installed in the system or not and its version and it is check using 
	:func:`rnaseqpipe.config.set_config.check_install`. 
	...
	for soft, row in dependencies_pd.iterrows():
		...
		if (message == 'OK'):
			...
			print ("+ Please install manually software: ", soft_name, " to continue with rnaseqpipe\n\n")
...
def get_python_packages(Debug):
	...
	It retrieves the dependencies name conversion from file :file:`rnaseqpipe/config/python/python_requirements.csv`
	using function :func:`rnaseqpipe.config.extern_progs.file_list` and :func:`rnaseqpipe.scripts.functions.main_functions.get_data`.
	For each module it retrieves the package version installed in the system using 
	:func:`rnaseqpipe.config.set_config.check_package_version`.	
...
def check_python_packages(Debug):
	...
	It uses function :func:`rnaseqpipe.config.set_config.get_python packages` to
	retrieve the version of the python packages installed in the system. Then it uses
	:func:`rnaseqpipe.config.extern_progs.min_python_module_version` to retrieve the minimum
	version specified. It compares them using function :func:`rnaseqpipe.config.set_config.check_install_module`.
	...
	for each in my_packages_requirements:
		...
		if (message == 'OK'):
			...
			print ("+ Please install manually package: ", each, " to continue with rnaseqpipe\n\n")
	...
	for index,row in packages.iterrows():
		...
		if (code=='OK'):
			...
			if (code2):
				...
				print ("Please install module %s manually to continue with rnaseqpipe" %index)
...
def check_install_module(installed, module_name, min_version, tag):
	...
	It prints messages using :func:`rnaseqpipe.config.set_config.print_module_comparison`.
