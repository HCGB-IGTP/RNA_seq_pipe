...
def file_list(wanted_data):
	"""
	Retrieves information of additional files under folder ``RSP/config``.
...
def read_dependencies():
	...
	It uses :func:`RSP.config.extern_progs.file_list` to retrieve absolute path
	for file :file:`RSP/config/software/dependencies.csv`. It then reads csv into pandas
	dataframe using :func:`RSP.scripts.functions.main_functions.get_data` and returns it.	
	"""
...
def return_defatult_soft(soft):
	...
	:file:`RSP/config/software/dependencies.csv` using :func:`RSP.config.extern_progs.read_dependencies`
	...
	.. seealso:: This function depends on other RSP functions:

		- :func:`RSP.config.extern_progs.read_dependencies`
...
def return_min_version_soft(soft):
	...
	Retrieves minimum version for the software of interest stored in :file:`RSP/config/software/dependencies.csv`.
	It reads file using :func:`RSP.config.extern_progs.read_dependencies`
...
def min_python_module_version():
	...
	Reads information from :file:`RSP/config/python/python_requirements.csv`.
...
def return_min_version_python_package(package):
	...
	It retrieves the requirements using :func:`RSP.config.extern_progs.min_python_module_version`
...
def print_package_version():
	"""
	Prints the package version required by ``RSP``

	It retrieves the requirements using :func:`RSP.config.extern_progs.min_python_module_version`
	and prints them using function :func:`RSP.config.set_config.print_module_comparison`.
